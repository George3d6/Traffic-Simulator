...
class CollectorActor() extends Actor{
  ...
  def receive = {
    case (point : Point, speed : Double, timestamp : Double, position : String, carId : BigInt) => {
      ...
      KafkaMessenger.sendToKafka(carId,
        s""" {"latitude": "$lat", "longitude" : "$lon",
           | "speed" : "$speed", "date" : "$date", "position": "$position", "id" : "$carId"} """.stripMargin)
    }
    case _      => {
      log.info(s"Collector received unknown message")
    }
  }
}